...
static void regptr (FILE *f, unsigned pc, unsigned r)
{
  if (!(ptr_mask & (1u << r)))
    {
      fprintf (f, "  %%r%x_%x = getelementptr [16 x i8], ptr @regs, i32 0, i32 %u\n", pc, r, r);
      ptr_mask |= 1u << r;
    }
}
...
static void loadreg (FILE *f, unsigned pc, unsigned r)
{
  ...
  if (!(load_mask & (1u << r)))
    {
      fprintf (f, "  %%v%x_%x = load i8, ptr %%r%x_%x\n", pc, r, pc, r);
      load_mask |= 1u << r;
    }
}
...
static void block (FILE *f, unsigned pc, uint16_t op)
{
  ...
  if (op == 0x00e0)
    {
      fprintf (f, "  call void @chip8_aot_clear()\n");
      next (f, pc);
      return;
    }
  if (op == 0x00ee)
    {
      fprintf (f, "  call void @chip8_aot_return()\n  br label %%dispatch\n\n");
      return;
    }
  switch (op >> 12)
    {
    ...
    case 1:
      ...
      break;
    case 2:
      fprintf (f, "  call void @chip8_aot_call(i16 %u, i16 %u)\n  br label %%dispatch\n\n", nnn, pc + 2);
      break;
    case 3:
    case 4:
      loadreg (f, pc, x);
      fprintf (f, "  %%c%x = icmp %s i8 %%v%x_%x, %u\n  %%p%x = select i1 %%c%x, i16 %u, i16 %u\n  store i16 %%p%x, ptr @program_counter\n  br label %%dispatch\n\n", pc, (op >> 12) == 3 ? "eq" : "ne",
	       pc, x, kk, pc, pc, pc + 4, pc + 2, pc);
      break;
    case 5:
    case 9:
      loadreg (f, pc, x);
      loadreg (f, pc, y);
      fprintf (f, "  %%c%x = icmp %s i8 %%v%x_%x, %%v%x_%x\n  %%p%x = select i1 %%c%x, i16 %u, i16 %u\n  store i16 %%p%x, ptr @program_counter\n  br label %%dispatch\n\n", pc,
	       (op >> 12) == 5 ? "eq" : "ne", pc, x, pc, y, pc, pc, pc + 4, pc + 2, pc);
      break;
    case 6:
      regptr (f, pc, x);
      fprintf (f, "  store i8 %u, ptr %%r%x_%x\n", kk, pc, x);
      next (f, pc);
      break;
    case 7:
      loadreg (f, pc, x);
      fprintf (f, "  %%a%x = add i8 %%v%x_%x, %u\n", pc, pc, x, kk);
      regptr (f, pc, x);
      fprintf (f, "  store i8 %%a%x, ptr %%r%x_%x\n", pc, pc, x);
      next (f, pc);
      break;
    case 8:
      fprintf (f, "  call void @chip8_aot_alu(i8 %u, i8 %u, i8 %u)\n", x, y, n);
      next (f, pc);
      break;
    case 0xa:
      fprintf (f, "  store i16 %u, ptr @addr\n", nnn);
      next (f, pc);
      break;
    case 0xb:
      loadreg (f, pc, 0);
      fprintf (f, "  %%z%x = zext i8 %%v%x_0 to i16\n  %%j%x = add i16 %u, %%z%x\n  store i16 %%j%x, ptr @program_counter\n  br label %%dispatch\n\n", pc, pc, pc, nnn, pc, pc);
      break;
    case 0xc:
      fprintf (f, "  call void @chip8_aot_random(i8 %u, i8 %u)\n", x, kk);
      next (f, pc);
      break;
    case 0xd:
      fprintf (f, "  call void @chip8_aot_draw(i8 %u, i8 %u, i8 %u)\n", x, y, n);
      next (f, pc);
      break;
    case 0xe:
      fprintf (f, "  call void @chip8_aot_key(i8 %u, i1 %u)\n  br label %%dispatch\n\n", x, (op & 255) == 0xa1);
      break;
    case 0xf:
      fprintf (f, "  call void @chip8_aot_f(i8 %u, i8 %u)\n", x, op & 255);
      next (f, pc);
      break;
    default:
      fprintf (f, "  call void @chip8_aot_bad(i16 %u, i16 %u)\n  br label %%exit\n\n", op, pc);
    }
}
...
int main (int ac, char **av)
{
  ...
  size_t z;
  FILE *in, *out;
  if ((ac != 4 && ac != 6) || strcmp (av[2], "-o") || (ac == 6 && strcmp (av[4], "--target")))
    {
  ...
  if (ferror (in) || fgetc (in) != EOF)
    die ("invalid ROM");
  fclose (in);
  if (!z || (z & 1))
    die ("ROM size must be a nonzero even number");
  if (!(out = fopen (av[3], "w")))
    die (strerror (errno));
  ...
  fprintf (out, "@chip8_aot_rom = constant [%zu x i8] [", z);
  for (size_t i = 0; i < z; i++)
    fprintf (out, "%si8 %u", i ? ", " : "", rom[i]);
  fprintf (out,
  ...
  for (size_t i = 0; i < z; i += 2)
    fprintf (out, "    i16 %zu, label %%pc%zx\n", ENTRYPOINT + i, ENTRYPOINT + i);
  fprintf (out, "  ]\n\n");
  for (size_t i = 0; i < z; i += 2)
    block (out, ENTRYPOINT + i, (rom[i] << 8) | rom[i + 1]);
  fprintf (out, "exit: ret void\n}\n");
  if (fclose (out))
    die ("cannot write LLVM IR");
  return 0;
}
