...
int chip8_aot_retire(void) { ++retired; poll_keys(); if ((retired & 31u) == 0) service_timers(); return (keys() & (1u<<31)) != 0; }
...
void chip8_aot_draw(uint8_t x,uint8_t y,uint8_t n) { uint8_t sprite[16]; int current_tick; poll_keys(); service_timers(); for(unsigned i=0;i<n;i++)sprite[i]=MEM_AT(addr+i);FLAGS=draw_io(regs[x],regs[y],n,sprite); /* The interpreter presents one frame per 60 Hz tick. Without this, ROMs such as UFO run at host speed. */ while ((current_tick = tick()) == last_tick) usleep(NANOS_PER_TICK >> 10); last_tick = current_tick; refresh_io(); }
void chip8_aot_key(uint8_t x,int up) { int down=(keys()&(1u<<regs[x]))!=0;if((down^up)) { clear_key(regs[x]); program_counter+=2; } program_counter+=2; }
