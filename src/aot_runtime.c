...
int chip8_aot_retire(void) { ++retired; poll_keys(); if ((retired & 31u) == 0) { int now=(int)(clock()/(CLOCKS_PER_SEC/60)); if(now!=last_tick) { if(delay_timer)--delay_timer; if(sound_timer)--sound_timer; last_tick=now; } } return (keys() & (1u<<31)) != 0; }
