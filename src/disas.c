...
analyze (size_t size)
{
  ...
  for (unsigned pc = ENTRYPOINT; pc + 1 < end; pc += 2)
	...
	case 0xb:
	  ends_block = true;
	  break;		/* computed jump: targets unknown */
	case 0xe:
	  if (kk == 0x9e || kk == 0xa1)
	    skip = true;
	  break;
      ...
      if (skip)
	{			/* fall-through and taken (pc+4) paths both start blocks */
	  if (pc + 2 < end)
	    leader[pc + 2] = true;
	  if (pc + 4 < end)
	    leader[pc + 4] = true;
	}
      if (ends_block && pc + 2 < end)
	leader[pc + 2] = true;
    }
}
  ...
  while (sp)
      ...
      while (pc + 1 < end && !reach[pc])
	{
	  ...
	  switch (op >> 12)
	    ...
	    case 0x9:
	      if (pc + 4 < end)
		stack[sp++] = pc + 4;	/* taken path; fall through */
	      break;
	    case 0xa:
	      last_ldi = nnn;
	      have_ldi = true;
	    ...
	    case 0xb:
	      ...
	      if (labelable (nnn, size))
		stack[sp++] = nnn;	/* V0 == 0 entry */
	      break;
	    case 0xd:
	      if (have_ldi)
		mark_data (last_ldi, n, size);
	      break;
	    case 0xe:
	      if (pc + 4 < end)
		stack[sp++] = pc + 4;
	    ...
	    case 0xf:
	      if (kk == 0x33)
		{
		  note_write (have_ldi, last_ldi, 3);
		  if (have_ldi)
		    mark_data (last_ldi, 3, size);
		}
	      if (kk == 0x55)
		{
		  note_write (have_ldi, last_ldi, x + 1);
		  if (have_ldi)
		    mark_data (last_ldi, x + 1, size);
		}
	      if (have_ldi && kk == 0x65)
		mark_data (last_ldi, x + 1, size);
	      break;
...
glue_unreachable (size_t size)
{
  ...
  for (unsigned pc = ENTRYPOINT; pc < end;)
      ...
      if (reach[pc])
	{
      ...
      unsigned start = pc;
      while (pc < end && !reach[pc])
	++pc;
      if (labelable (start, size))
	leader[start] = true;	/* labels only exist at aligned, in-ROM addresses */
      for (unsigned a = start + 1; a < pc; ++a)
	leader[a] = false;
    }
}
...
print_report (size_t size)
{
  ...
  for (unsigned pc = ENTRYPOINT; pc < end;)
    {
      ...
      while (run < end && class[run] == c)
	++run;
      totals[c] += run - pc;
      fprintf (stderr, "  0x%03X-0x%03X  %-7s (%u bytes)\n", pc, run - 1, names[c], run - pc);
      pc = run;
    }
  ...
  if (smc_possible && !assume_no_smc)
    fprintf (stderr, "warning: reachable memory writes with dynamic or "
	     "code-overlapping targets; self-modifying code may invalidate " "this map (re-run with --assume-no-smc to assert otherwise)\n");
  if (assume_no_smc)
...
#define BOTH(asm_format, prose_format, ...) \
  ...
  switch (op >> 12)
    ...
    case 0x0:
      if (assembly_output)
	raw_word (pc, op);	/* SYS is unsupported by this emulator. */
      else
	output (pc, "jump 0x%04X", nnn);
      return;
    case 0x1:
      if (assembly_output && leader[nnn])
	{
	  output (pc, "JP block%03X", nnn);
	  return;
	}
    ...
    case 0x2:
      if (assembly_output && leader[nnn])
	{
	  output (pc, "CALL block%03X", nnn);
	  return;
	}
      BOTH ("CALL 0x%03X", "call 0x%04X", nnn);
    case 0x3:
      BOTH ("SE V%X, 0x%02X", "skip next if V%X == 0x%04X", x, kk);
    case 0x4:
      BOTH ("SNE V%X, 0x%02X", "skip next if V%X != 0x%04X", x, kk);
    case 0x5:
      if (n == 0 || !assembly_output)
	BOTH ("SE V%X, V%X", "skip next if V%X == V%X", x, y);
      raw_word (pc, op);
      return;
    case 0x6:
      BOTH ("LD V%X, 0x%02X", "V%X = 0x%04X", x, kk);
    case 0x7:
      BOTH ("ADD V%X, 0x%02X", "V%X += 0x%04X", x, kk);
    case 0x8:
      switch (n)
	{
	case 0:
	  BOTH ("LD V%X, V%X", "V%X = V%X", x, y);
	case 1:
	  BOTH ("OR V%X, V%X", "V%X |= V%X", x, y);
	case 2:
	  BOTH ("AND V%X, V%X", "V%X &= V%X", x, y);
	case 3:
	  BOTH ("XOR V%X, V%X", "V%X ^= V%X", x, y);
	case 4:
	  BOTH ("ADD V%X, V%X", "V%X += V%X", x, y);
	case 5:
	  BOTH ("SUB V%X, V%X", "V%X -= V%X", x, y);
	case 6:
	  BOTH ("SHR V%X, V%X", "V%X >>= V%X", x, y);
	case 7:
	  BOTH ("SUBN V%X, V%X", "V%X = V%X - V%X", x, y, x);
	case 0xe:
	  BOTH ("SHL V%X, V%X", "V%X <<= V%X", x, y);
	default:
	  raw_word (pc, op);
	  return;
	}
    case 0x9:
      if (n == 0 || !assembly_output)
	BOTH ("SNE V%X, V%X", "skip next if V%X != V%X", x, y);
      raw_word (pc, op);
      return;
    case 0xa:
      if (assembly_output && leader[nnn])
	{
	  output (pc, "LD I, block%03X", nnn);
	  return;
	}
	...
	output (pc, "branch to 0x%04X + V0", nnn);
      return;
    case 0xc:
      BOTH ("RND V%X, 0x%02X", "V%X = <random> & 0x%04X", x, kk);
    case 0xd:
      BOTH ("DRW V%X, V%X, 0x%X", "draw sprite from addr of height %d at (V%X, V%X)", x, y, n);
    case 0xe:
      if (kk == 0x9e)
	BOTH ("SKP V%X", "skip next if V%X is down", x);
      if (kk == 0xa1)
	BOTH ("SKNP V%X", "skip next if V%X is up", x);
      raw_word (pc, op);
      return;
    case 0xf:
      switch (kk)
	{
	case 0x07:
	  BOTH ("LD V%X, DT", "V%X = <current delay timer>", x);
	case 0x0a:
	  BOTH ("LD V%X, K", "V%X = <next key pressed>", x);
	case 0x15:
	  BOTH ("LD DT, V%X", "<current delay timer> = V%X", x);
	case 0x18:
	  BOTH ("LD ST, V%X", "<current sound timer> = V%X", x);
	case 0x1e:
	  BOTH ("ADD I, V%X", "addr += V%X", x);
	case 0x29:
	  BOTH ("LD F, V%X", "addr = <address of hex sprite V%X>", x);
	case 0x33:
	  BOTH ("LD B, V%X", "store V%X in BCD starting at addr", x);
	case 0x55:
	  BOTH ("LD [I], V%X", "save registers V0 through V%X starting at addr", x);
	case 0x65:
	  BOTH ("LD V%X, [I]", "load registers V0 through V%X starting at addr", x);
	default:
	  raw_word (pc, op);
	  return;
	}
...
main (int argc, const char *argv[])
{
  ...
  for (int i = 1; i < argc; ++i)
	...
	report = true;
      else if (!strcmp (argv[i], "--assume-no-smc"))
	assume_no_smc = true;
      else if (!strcmp (argv[i], "--decode-unreachable"))
	decode_unreachable = true;
  ...
  for (unsigned pc = ENTRYPOINT; pc + 1 < ENTRYPOINT + size; pc += 2)
    {
      uint16_t op = (uint16_t) ((memory[pc] << 8) | memory[pc + 1]);
      if (assembly_output && leader[pc])
	fprintf (stdout, "block%03X:\n", pc);
      if (!decode_unreachable && !reach[pc])
	{			/* dead word: emit as data */
	  if (assembly_output)
	    fprintf (stdout, "        .word 0x%04X ; data (unreachable)\n", op);
	  else
	    output (pc, "data word %04X", op);
	  continue;
	}
      disassemble (pc, op);
    }
  if (size & 1)
    {				/* odd-sized ROM: preserve the trailing byte */
	...
	output (pc, "trailing byte %02X at pc 0x%04X", memory[pc], pc);
    }
  return 0;
}
