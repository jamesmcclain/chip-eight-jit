...
static gcc_jit_rvalue *snapshot (gcc_jit_context *ctx, gcc_jit_function *fn, gcc_jit_block *blk, gcc_jit_type *u8, gcc_jit_type *u8p, void *host_addr, int id)
{
  ...
  gcc_jit_block_add_assignment (blk, NULL, local, gcc_jit_lvalue_as_rvalue (mem (ctx, u8p, host_addr)));
...
code codegen (void)
{
  ...
  gcc_jit_function *function = gcc_jit_context_new_function (ctx, NULL, GCC_JIT_FUNCTION_EXPORTED,
							     t_void, fn_name, 0, NULL, 0);

  // Imported host helpers.
  gcc_jit_function *host[N_HOST_FNS];
  for (int i = 0; i < N_HOST_FNS; ++i)
    host[i] = gcc_jit_context_new_function (ctx, NULL, GCC_JIT_FUNCTION_IMPORTED, t_void, HOST_FNS[i], 0, NULL, 0);
  ...
  for (uint16_t pc = program_counter, op_count = 0;; pc += 2, ++op_count)
	...
	case 0x1:
	  {			// jump
	    IMMEDIATE12;
	    SAFEPOINT ();
	    gcc_jit_block_add_assignment (blk, NULL, PC_LVAL, gcc_jit_context_new_rvalue_from_int (ctx, t_u16, immediate));
	...
	case 0x9:		// skip if Vx != Vy
	  {
	    ...
	    enum gcc_jit_comparison cmp = (((op & 0xf000) >> 12) == 0x3 || ((op & 0xf000) >> 12) == 0x5) ? GCC_JIT_COMPARISON_EQ : GCC_JIT_COMPARISON_NE;
		...
		SAFEPOINT ();
		gcc_jit_block_add_assignment (blk, NULL, PC_LVAL, gcc_jit_context_new_rvalue_from_int (ctx, t_u16, fused_target));
		...
		gcc_jit_block_add_assignment (else_blk, NULL, PC_LVAL, gcc_jit_context_new_rvalue_from_int (ctx, t_u16, (uint16_t) (pc + 2)));
		gcc_jit_block_end_with_void_return (else_blk, NULL);
	      }

	    // "skip": materialize its successor and keep compiling inline.
	    blk = then_blk;
	    gcc_jit_block_add_assignment (blk, NULL, PC_LVAL, gcc_jit_context_new_rvalue_from_int (ctx, t_u16, (uint16_t) (pc + 4)));
	...
	case 0x6:
	  {			// Vx = imm
	    X;
	    IMMEDIATE8;
	    gcc_jit_block_add_assignment (blk, NULL, mem (ctx, t_u8p, &regs[x]), gcc_jit_context_new_rvalue_from_int (ctx, t_u8, immediate));
	    STEP_AND_CONTINUE;
	  }
	case 0x7:
	  {			// Vx += imm
	    X;
	    IMMEDIATE8;
	    gcc_jit_block_add_assignment_op (blk, NULL, mem (ctx, t_u8p, &regs[x]), GCC_JIT_BINARY_OP_PLUS, gcc_jit_context_new_rvalue_from_int (ctx, t_u8, immediate));
	    STEP_AND_CONTINUE;
	  }
	case 0x8:
	  {
	    ...
	    switch (op & 0x000f)
	      {
	      case 0x0:	// Vx = Vy
		gcc_jit_block_add_assignment (blk, NULL, rx, gcc_jit_lvalue_as_rvalue (mem (ctx, t_u8p, &regs[y])));
		STEP_AND_CONTINUE;
	      case 0x1:	// Vx |= Vy
		gcc_jit_block_add_assignment_op (blk, NULL, rx, GCC_JIT_BINARY_OP_BITWISE_OR, gcc_jit_lvalue_as_rvalue (mem (ctx, t_u8p, &regs[y])));
		STEP_AND_CONTINUE;
	      case 0x2:	// Vx &= Vy
		gcc_jit_block_add_assignment_op (blk, NULL, rx, GCC_JIT_BINARY_OP_BITWISE_AND, gcc_jit_lvalue_as_rvalue (mem (ctx, t_u8p, &regs[y])));
		STEP_AND_CONTINUE;
	      case 0x3:	// Vx ^= Vy
		gcc_jit_block_add_assignment_op (blk, NULL, rx, GCC_JIT_BINARY_OP_BITWISE_XOR, gcc_jit_lvalue_as_rvalue (mem (ctx, t_u8p, &regs[y])));
		STEP_AND_CONTINUE;
	      case 0x4:
		{		// Vx += Vy, VF = carry
		  ...
		  gcc_jit_block_add_assignment (blk, NULL, rx, gcc_jit_context_new_cast (ctx, NULL, sum16, t_u8));
		  gcc_jit_block_add_assignment (blk, NULL, mem (ctx, t_u8p, &regs[0xf]), gcc_jit_context_new_cast (ctx, NULL, carry, t_u8));
		  STEP_AND_CONTINUE;
		}
	      case 0x5:
		{		// Vx -= Vy, VF = NOT borrow
		  ...
		  gcc_jit_block_add_assignment (blk, NULL, rx, gcc_jit_context_new_binary_op (ctx, NULL, GCC_JIT_BINARY_OP_MINUS, t_u8, vx, vy));
		  gcc_jit_block_add_assignment (blk, NULL, mem (ctx, t_u8p, &regs[0xf]), gcc_jit_context_new_cast (ctx, NULL, gt, t_u8));
		  STEP_AND_CONTINUE;
		}
	      case 0x6:
		{		// Vx >>= 1, VF = lost low bit
		  ...
		  gcc_jit_block_add_assignment (blk, NULL, rx, gcc_jit_context_new_binary_op (ctx, NULL, GCC_JIT_BINARY_OP_RSHIFT, t_u8, vx, one));
		  gcc_jit_block_add_assignment (blk, NULL, mem (ctx, t_u8p, &regs[0xf]), gcc_jit_context_new_binary_op (ctx, NULL, GCC_JIT_BINARY_OP_BITWISE_AND, t_u8, vx, one));
		  STEP_AND_CONTINUE;
		}
	      case 0x7:
		{		// Vx = Vy - Vx, VF = NOT borrow
		  ...
		  gcc_jit_block_add_assignment (blk, NULL, rx, gcc_jit_context_new_binary_op (ctx, NULL, GCC_JIT_BINARY_OP_MINUS, t_u8, vy, vx));
		  gcc_jit_block_add_assignment (blk, NULL, mem (ctx, t_u8p, &regs[0xf]), gcc_jit_context_new_cast (ctx, NULL, gt, t_u8));
		  STEP_AND_CONTINUE;
		}
	      case 0xe:
		{		// Vx <<= 1, VF = lost high bit
		  ...
		  gcc_jit_block_add_assignment (blk, NULL, rx, gcc_jit_context_new_binary_op (ctx, NULL, GCC_JIT_BINARY_OP_LSHIFT, t_u8, vx, one));
		  gcc_jit_block_add_assignment (blk, NULL, mem (ctx, t_u8p, &regs[0xf]), gcc_jit_context_new_binary_op (ctx, NULL, GCC_JIT_BINARY_OP_RSHIFT, t_u8, hi, seven));
		  STEP_AND_CONTINUE;
		}
	      default:
		BAIL_ERRER;
	...
	case 0xa:
	  {			// addr = imm
	    IMMEDIATE12;
	    gcc_jit_block_add_assignment (blk, NULL, mem (ctx, t_u16p, &addr), gcc_jit_context_new_rvalue_from_int (ctx, t_u16, immediate));
	    STEP_AND_CONTINUE;
	  }
	case 0xb:
	  {			// jump to imm + V0
	...
	case 0xf:
	  {
...
#endif
		  gcc_jit_block_add_assignment (blk, NULL, mem (ctx, t_u8p, &regs[x]), gcc_jit_lvalue_as_rvalue (mem (ctx, t_u8p, &delay_timer)));
		  STEP_AND_CONTINUE;
		}
	      case 0x0a:
		...
		{		// delay_timer = Vx
		  ...
		  CALL_HOST ("sync_timers");	// read/write the timer at the current clock
#endif
		  gcc_jit_block_add_assignment (blk, NULL, mem (ctx, t_u8p, &delay_timer), gcc_jit_lvalue_as_rvalue (mem (ctx, t_u8p, &regs[x])));
		  STEP_AND_CONTINUE;
		}
	      case 0x18:
		{		// sound_timer = Vx
		  ...
		  CALL_HOST ("sync_timers");	// read/write the timer at the current clock
#endif
		  gcc_jit_block_add_assignment (blk, NULL, mem (ctx, t_u8p, &sound_timer), gcc_jit_lvalue_as_rvalue (mem (ctx, t_u8p, &regs[x])));
		  STEP_AND_CONTINUE;
		}
	      case 0x1e:
		{		// addr += Vx
		  X;
		  gcc_jit_rvalue *vx = gcc_jit_lvalue_as_rvalue (mem (ctx, t_u8p, &regs[x]));
		  gcc_jit_block_add_assignment_op (blk, NULL, mem (ctx, t_u16p, &addr), GCC_JIT_BINARY_OP_PLUS, gcc_jit_context_new_cast (ctx, NULL, vx, t_u16));
		  STEP_AND_CONTINUE;
		}
	      case 0x29:
